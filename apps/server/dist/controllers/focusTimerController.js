...
exports.getByGoalStats = exports.getLast7DaysStats = exports.deleteFocusTimer = exports.updateFocusTimer = exports.createFocusTimer = exports.getFocusTimers = void 0;
...
exports.getFocusTimers = (0, asyncWrapper_1.wrapAsync)(async (req, res) => {
    const query = { user: req.user?.id };
    if (req.query?.linkType) {
        const linkType = String(req.query.linkType).trim();
        if (["goal", "personal"].includes(linkType))
    ...
    const limit = Math.max(1, Math.min(200, Number.isFinite(Number(limitRaw)) ? Number(limitRaw) : 100));
    const focusTimers = await FocusTimer_1.default.find(query)
...
exports.createFocusTimer = (0, asyncWrapper_1.wrapAsync)(async (req, res) => {
    const { title, durationSeconds, goalTag, mode, status, startedAt, endedAt } = req.body;
    const trimmedTitle = String(title || "").trim();
    if (!trimmedTitle)
        throw new ExpressError_1.default(400, "Title is required");
    const seconds = Number(durationSeconds);
    if (!Number.isFinite(seconds) || seconds < 1) {
        throw new ExpressError_1.default(400, "durationSeconds must be a positive number");
    }
    ...
    if (mode && !["focus", "shortBreak", "longBreak"].includes(nextMode)) {
        throw new ExpressError_1.default(400, "Invalid mode");
    }
    const nextStatus = String(status || "completed").trim();
    if (status && !["completed", "cancelled"].includes(nextStatus)) {
        throw new ExpressError_1.default(400, "Invalid status");
    }
    ...
    if (startedAtDate == null || endedAtDate == null) {
        throw new ExpressError_1.default(400, "startedAt or endedAt is null");
    }
    const newFocusTimer = new FocusTimer_1.default({
        title: trimmedTitle,
    ...
    const populatedTimer = await FocusTimer_1.default.findById(svdFocusTimer._id).populate("goal", "title tag");
    res.status(201).json({
        success: true,
...
exports.updateFocusTimer = (0, asyncWrapper_1.wrapAsync)(async (req, res) => {
    const { id } = req.params;
    if (!mongoose_1.default.isValidObjectId(id)) {
        throw new ExpressError_1.default(400, "Invalid timer id");
    }
    const timer = await FocusTimer_1.default.findOne({ _id: id, user: req.user?.id });
    if (!timer)
        throw new ExpressError_1.default(404, "Timer not found");
    if (typeof req.body?.title === "string") {
        const t = req.body.title.trim();
        if (!t)
            throw new ExpressError_1.default(400, "Title cannot be empty");
        timer.title = t;
    }
    ...
    if (req.body?.startedAt !== undefined) {
        ...
        if (req.body.startedAt && (d == null || Number.isNaN(d.getTime()))) {
            throw new ExpressError_1.default(400, "Invalid startedAt");
        }
        timer.startedAt = d;
    }
    if (req.body?.endedAt !== undefined) {
        ...
        if (!d || Number.isNaN(d.getTime())) {
            throw new ExpressError_1.default(400, "Invalid endedAt");
        }
        timer.endedAt = d;
    }
    ...
    await timer.save();
    const updatedTimer = await FocusTimer_1.default.findById(timer._id).populate("goal", "title tag");
    if (!updatedTimer)
        throw new ExpressError_1.default(404, "Timer not found");
    res.status(200).json({
        success: true,
...
exports.deleteFocusTimer = (0, asyncWrapper_1.wrapAsync)(async (req, res) => {
    const { id } = req.params;
    if (!mongoose_1.default.isValidObjectId(id)) {
        throw new ExpressError_1.default(400, "Invalid timer id");
    }
    const deleted = await FocusTimer_1.default.findOneAndDelete({
        _id: id,
        user: req.user?.id,
    });
    if (!deleted)
...
exports.getLast7DaysStats = (0, asyncWrapper_1.wrapAsync)(async (req, res) => {
    ...
    const agg = await FocusTimer_1.default.aggregate([
        {
            $match: {
                user: userId,
                mode: "focus",
    ...
    const startLocalShifted = new Date(startOfRangeUtc.getTime() - tzOffsetMinutes * 60 * 1000);
...
exports.getByGoalStats = (0, asyncWrapper_1.wrapAsync)(async (req, res) => {
    const tzOffsetMinutes = parseTzOffsetMinutes(req);
    const now = new Date();
    const startOfTodayUtc = getStartOfLocalDayUtc(now, tzOffsetMinutes);
    const startOfRangeUtc = new Date(startOfTodayUtc.getTime() - 6 * 86400000);
    const endUtc = new Date(startOfTodayUtc.getTime() + 86400000);
    const userId = new mongoose_1.default.Types.ObjectId(req.user?.id);
    const rows = await FocusTimer_1.default.aggregate([
        {
            $match: {
                user: userId,
                mode: "focus",
